  ...
  async.waterfall([
    ...
    function createInstance(cb) {
      ...
      p.on('exit', function(code, signal) {
        ...
        var instanceUrl = output.match(/"dnsName": "([a-z0-9\-\.]+)",/)[1];
        ...
        p.on('exit', function(code, signal) {
          ...
          // See https://github.com/mozilla/awsbox/issues/43
          var remoteCmd = 'cd /home/app/git \\&\\& ./hooks/post-update';
          remoteCmd = 'su -l app -c \\"' + remoteCmd + '\\"';
          remoteCmd = 'echo ' + remoteCmd + ' >> /etc/rc.local';
          remoteCmd = 'sudo /bin/bash -c \'' + remoteCmd + '\'';
          var p = child_process.spawn('ssh', ['-o', 'StrictHostKeyChecking no',
                                      'ec2-user@' + instanceUrl, remoteCmd],
                                      {stdio: 'inherit', cwd: workDir});
          p.on('exit', function(code, signal) {
            var err = code || signal;
            return cb(err);
