...
function createInstance(state, cb) {
  ...
  getConnection(state.opts, function(err, ec2) {
    ...
    async.waterfall([
      ...
      function runBuildScript(instanceId, instanceURL, cb) {
        var buildScript = boxProps.BuildScript;
        ...
        var args = ['-v', '-o', 'StrictHostKeyChecking no',
        ...
        p.on('exit', function(code, signal) {
          ...
          var cmd = 'sudo chmod +x /tmp/buildit && sudo /tmp/buildit';
          var args = ['-o', 'StrictHostKeyChecking no', remoteHost, cmd];
