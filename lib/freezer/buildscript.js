  ...
  getConnection(state.opts, function(err, ec2) {
    ...
    async.waterfall([
      ...
      function runBuildScripts(instanceId, instanceURL, cb) {
        var buildScripts = boxProps.BuildScripts || [];
        if (boxProps.BuildScript) {
            buildScripts.push(boxProps.BuildScript);
        }
        async.eachSeries(buildScripts, function(buildScript, cb) {
          buildScript = path.join(state.workDir, buildScript);
          console.log("Copying build script:", buildScript);
          var remoteHost =  boxProps.User + '@' + instanceURL;
          var args = ['-o', 'StrictHostKeyChecking no',
                      buildScript, remoteHost + ':/tmp/buildit'];
          ...
          p.on('exit', function(code, signal) {
            ...
            var args = ['-t', '-o', 'StrictHostKeyChecking no', remoteHost, cmd];
...
function calcConfigHash(state, cb) {
  ...
  async.eachSeries(buildScripts, function(buildScript, cb) {
    buildScript = path.join(state.workDir, buildScript);
    fs.readFile(buildScript, function(err, data) {
      if (err) return cb(err);
      configHasher.update(data);
      return cb(null);
    });
  }, function(err) {
    if (err) return cb(err);
