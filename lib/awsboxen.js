...
'\n'+
'Usage:\n'+
'  awsboxen [options] deploy [-D PARAM...] <stack-name>\n'+
'  awsboxen [options] freeze [-D PARAM...] [<boxen>...]\n'+
'  awsboxen [options] showconfig\n'+
'  awsboxen [options] list\n'+
'  awsboxen [options] info <stack-name>\n'+
...
module.exports.commands = {
  ...
  info: function list(opts, cb) {
    cfn.checkStack(opts, function(err, stack) {
      if (err) return cb(err);
      if (!stack) return cb('no such stack: "' + opts.stack_name + '"');
      console.log('Info for ' + opts.stack_name + ':');
      for (var k in stack) {
        if(stack.hasOwnProperty(k)) {
          console.log('  ' + k + ':  ' + stack[k]);
        }
