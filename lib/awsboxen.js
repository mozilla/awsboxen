...
module.exports.commands = {
  ...
  deploy: function deploy(opts, cb) {
    async.waterfall([
      ...
      function loadTheConfig(cb) {
        projinfo('.', opts.profile, opts.ref).loadConfig(cb);
      },
      ...
      function generateBoxenAMIs(cfg, cb) {
        async.eachSeries(Object.keys(cfg.Boxen), function(boxName, cb) {
          ...
          freezer.createAwsBoxAMI(opts, '.', boxName, boxCfg, function(err, amiId) {
            if (err) return cb(err);
            // XXX TODO: we probably can't use "::" in parameter names; sadface
            cfg.Parameters['Boxen::' + boxName] = {
              Type: 'String',
              Default: amiId
            };
            cb(null);
  ...
  freeze: function freeze(opts, cb) {
    opts.boxName = opts.boxName || 'DefaultBox';
    freezer.createAwsBoxAMI(opts, '.', opts.boxName, null, function(err, amiId) {
      if (err) return cb(err);
